...
//? 相对于当前容器的边界算法
export const elementToContainerBoundary = (main: DOMRect, container: DOMRect, option: { direction: string; align: string }) => {
...
//? 相对于当前鼠标位置的边界算法
export const elementToMouseBoundary = (ev: MouseEvent, container: DOMRect, option: { offsetX: number; offsetY: number }) => {
    const translate = { x: "", y: "" };
    const innerWidth = window.innerWidth + window.scrollX;
    const innerHeight = window.innerHeight + window.scrollY;
    //* 数据初始化
