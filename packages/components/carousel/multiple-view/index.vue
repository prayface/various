<template>
    <div class="ui-mv-carousel" :class="className" :style="style" ref="main" v-bind="attrs">
        ...
        <!-- 轮播图左侧箭头 -->
        <div class="ui-mv-carousel-control ui-mv-carousel-left-control" :class="{ 'ui-disabled-status': (boundary == 'first') }" @click="switchCarousel(-1)" v-if="arrow != 'never' && controls">
            <UiIcon name="arrow" />
        </div>
        <!-- 轮播图右侧箭头 -->
        <div class="ui-mv-carousel-control ui-mv-carousel-right-control" :class="{ 'ui-disabled-status': (boundary == 'last') }" @click="switchCarousel(1)" v-if="arrow != 'never' && controls">
            <UiIcon name="arrow" />
        </div>
    </div>
</template>
...
const { main, controls, container, boundary } = refs;
